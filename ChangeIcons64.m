...
path=pwd;
BATstr=['REGEDIT /e ' path '\OrigReg64.reg "HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Explorer\ShellIconOverlayIdentifiers"'];
batfile=[path '\Overlay64.bat'];
fid = fopen(batfile, 'w');
fprintf(fid,'%s\n', BATstr);
fprintf(fid,'%s\n', 'exit');
fclose(fid);
system( 'Overlay64.bat')

%% read in original registry file
warning off MATLAB:iofun:UnsupportedEncoding;
fid = fopen('OrigReg64.reg', 'r', 'l', 'UTF-16');
i = 1;
tline = fgetl(fid);
A{i} = tline;
while ischar(tline)
...
Regstr2 = '';
Regstr3 = ['[-HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Explorer\ShellIconOverlayIdentifiers]']; %#ok<*NBRAK>
Regfile1=['RemoveReg64.reg'];
fid = fopen(Regfile1, 'w');
fprintf(fid,'%s\n', Regstr1);
fprintf(fid,'%s\n', Regstr2);
fprintf(fid,'%s\n', Regstr3);
fclose(fid);
system ('RemoveReg64.reg')
system ('Modified64.reg')
